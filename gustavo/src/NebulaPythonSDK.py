import requests, base64, json, six, os
...
class Nebula:
    ...
    def __init__(self, username=None, password=None, token=None, host="127.0.0.1", port=80, protocol="http",
                 host_uri=None, request_timeout=60,auth_enabled=False):
        ...
        self.auth_enabled = auth_enabled or str(os.getenv("AUTH_ENABLED", "false")).lower() in ("1", "true", "yes", "on")
        if host_uri:
            self.host = host_uri
        else:
        ...
        if not auth_enabled:
            ...
            self.headers["X-UID"] = self.username
            self.token = os.environ.get("GUSTAVO_USER_TOKEN","")
            self.headers["authorization"] = "Bearer " + self.token
