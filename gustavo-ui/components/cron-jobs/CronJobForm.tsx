...
const cronJobFormSchema = z.object({
  ...
  schedule: z.string().min(1, "Schedule is required").regex(
    /^\S+\s+\S+\s+\S+\s+\S+\s+\S+$/,
    "Must be a 5-field cron expression, e.g. */15 * * * *"
  ),
...
export type CronJobFormValues = z.infer<typeof cronJobFormSchema>;

interface CronJobFormProps {
  defaultValues?: Partial<CronJobFormValues>;
...
export function CronJobForm({ defaultValues, onSubmit, submitLabel = "Create Cron Job", isEdit = false, memberDeviceGroups = [] }: CronJobFormProps) {
    ...
    <form onSubmit={handleFormSubmit} className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor="name">Cron Job Name</Label>
          ...
          <div>
            ...
            <Input id="schedule" {...register("schedule")} className="mt-1 font-mono" placeholder="*/15 * * * *" />
            {errors.schedule && <p className="text-sm text-red-600 mt-1">{errors.schedule.message}</p>}
          </div>
          <div className="flex gap-6">
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" {...register("running")} /> Running (enabled)
            </label>
            <label className="flex items-center gap-2 text-sm">
      ...
      {/* Owner group — only relevant for non-admins creating a new cron job */}
      {!isEdit && !isAdmin && (
        <Card>
          <CardHeader>
          ...
          <CardContent>
            {myGroups.length === 0 ? (
              <p className="text-sm text-red-600">
                You&apos;re not a member of any group yet — ask an admin to add you to one before creating cron jobs.
              </p>
            ) : myGroups.length === 1 ? (
              <p className="text-sm text-muted-foreground">
                This cron job will be owned by your group <strong>{myGroups[0]}</strong>.
              </p>
            ) : (
              <div>
                <Label htmlFor="owner_group">Which group should own this cron job?</Label>
                <select
                  id="owner_group"
